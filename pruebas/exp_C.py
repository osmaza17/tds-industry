...
# FOCUS=window-features (instance C): sliding-window SHAPE descriptors of the
# temperature trace. The synthetic fault is a triangular thermal rise-and-fall;
# its SHAPE (skew/kurtosis/slope/range over a window) is far more invariant to the
# absolute temperature level than the raw value, so it should survive the temp<->fault
# inversion between the two real fault sequences better than absolute features.
WIN_SHAPE_WINDOWS = [20, 50]   # need enough points for moments / slope
EWMA_SPANS        = [10, 30]


def _rolling_slope(temp: pd.Series, w: int) -> pd.Series:
    """Vectorised least-squares slope of `temp` over a trailing window of length w.

    Equivalent to fitting y = a + b*t (t = 0..w-1 evenly spaced) inside each window and
    keeping b. Implemented as a single linear filter (centred-ramp correlation) so it is
    O(n) instead of a slow rolling.apply. Positions before the first full window are 0."""
    x = np.asarray(temp, dtype=float)
    n = len(x)
    if n < 2 or w < 2:
        return pd.Series(np.zeros(n), index=temp.index)
    t = np.arange(w, dtype=float)
    t -= t.mean()
    denom = (t * t).sum()
    kernel = t[::-1] / denom                      # see derivation: conv reproduces windowed slope
    conv = np.convolve(x, kernel, mode='full')[:n]
    conv[:w - 1] = 0.0
...
def temp_derived_features(temp: pd.Series) -> dict:
    ...
    # Window-shape features (instance C focus)
    for w in WIN_SHAPE_WINDOWS:
        r = temp.rolling(w, min_periods=2)
        slope = _rolling_slope(temp, w)
        ...
        # Direction-invariant magnitude of the local trend: identical whether the
        # thermal pulse rises or falls -> robust to the temp<->fault inversion in M1.
        out[f'temperature_absslope_{w}']   = slope.abs()
...
# ── Window-shape feature sets (instance C focus) ──────────────────────────────
# Full window-shape block: skew/kurt/slope/range over [20,50] + EWMA deviation.
WIN_FEATS_FULL = ['temperature_roll_skew_20', 'temperature_roll_kurt_20',
                  'temperature_roll_range_20', 'temperature_slope_20',
                  'temperature_roll_skew_50', 'temperature_roll_kurt_50',
                  'temperature_roll_range_50', 'temperature_slope_50',
                  'temperature_ewma_dev_10', 'temperature_ewma_dev_30']
# Leaner block: slope + range + skew (drop noisy kurtosis & one EWMA)
WIN_FEATS_LEAN = ['temperature_slope_20', 'temperature_slope_50',
                  'temperature_roll_range_20', 'temperature_roll_range_50',
                  'temperature_roll_skew_20', 'temperature_ewma_dev_30']

# Direction-invariant ("magnitude") window block: range + |slope| + kurtosis.
# All are identical whether the thermal pulse rises or falls -> principled fix for the
# temp<->fault sign inversion that destabilises M1. NO signed features here.
WIN_FEATS_MAG = ['temperature_roll_range_20', 'temperature_roll_range_50',
                 'temperature_absslope_20', 'temperature_absslope_50',
                 'temperature_roll_kurt_20']

# Champion base for M1 is DYN_FEATS; augment it with window-shape descriptors.
DYN_PLUS_WIN_FULL = DYN_FEATS + WIN_FEATS_FULL
DYN_PLUS_WIN_LEAN = DYN_FEATS + WIN_FEATS_LEAN
DYN_PLUS_WIN_MAG  = DYN_FEATS + WIN_FEATS_MAG

# Add window features to M1 only (the bottleneck)
FEATS_WIN_M1      = dict(FEATS_DYN14); FEATS_WIN_M1[1]      = DYN_PLUS_WIN_FULL
FEATS_WIN_M1_LEAN = dict(FEATS_DYN14); FEATS_WIN_M1_LEAN[1] = DYN_PLUS_WIN_LEAN
FEATS_WIN_M1_MAG  = dict(FEATS_DYN14); FEATS_WIN_M1_MAG[1]  = DYN_PLUS_WIN_MAG
# Add to both hard motors M1 and M4
FEATS_WIN_M14 = dict(FEATS_DYN14)
FEATS_WIN_M14[1] = DYN_PLUS_WIN_FULL
FEATS_WIN_M14[4] = DYN_PLUS_WIN_FULL
...
EXPERIMENTS = {
    ...
    # ── Instance C: window-shape features (FOCUS=window-features) ──────────────
    # c_base reproduces champ3 (single-seed) under tag C so M1/M4 numbers are an
    # honest A/B reference for the window-feature variants (only features change,
    # so RNG is identical motor-by-motor -> clean comparison).
    'c_base':     dict(features=FEATS_DYN14,     inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3),
    'c_win_m1':   dict(features=FEATS_WIN_M1,    inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3),
    'c_win_m1l':  dict(features=FEATS_WIN_M1_LEAN,inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3),
    'c_win_m1m':  dict(features=FEATS_WIN_M1_MAG, inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3),
    'c_win_m14':  dict(features=FEATS_WIN_M14,   inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3),
}
...
def main():
    ...
    parser.add_argument('--exclude-motor', type=int, choices=range(0, 7), default=3,
                        help="Motor forced to -1 for diagnostic probing; 0 = none (full submittable CSV)")
    ...
    for motor_id in range(1, 7):
        ...
        def make_aug(seed_offset):
            ...
            s = synthesize_for_motor(raw_train, motor_id, n_sequences=INJ[motor_id],
                                     peak_low=peak_low, peak_high=peak_high)
            return pd.concat([raw_train, s], ignore_index=True) if not s.empty else raw_train
    ...
    # Probe: force one motor to -1 so Kaggle leaks per-motor F1 (skip if 0 = full CSV)
    if args.exclude_motor != 0:
