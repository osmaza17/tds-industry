...
EXPERIMENTS = {
    ...
    # ── FOCUS A: threshold-cv (leave-one-fault-out OOF threshold + train-on-all) ──
    # cv_loo = champ3 base, but threshold/closing chosen by leave-one-fault-out OOF
    # and final model trained on ALL fault sequences. Single-seed final (threshold is
    # the lever under test; ensembling is orthogonal). Anchor = champ3.
    'cv_loo':     dict(features=FEATS_DYN14, inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3, cv_thresh=True),
    # cv_ens2: same LOO threshold, but DEPLOY the cross-fold ensemble (matched prob scale)
    # instead of retraining on all -> fixes the cv_loo threshold-transfer regression.
    'cv_ens2':    dict(features=FEATS_DYN14, inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3, cv_thresh=True, cv_deploy='folds'),
    # cv_wgt: fault-count-weighted per-fold F1 (concentrated-fault motors drive threshold),
    # deploy on all data (sharper -> less conservative, recovers M4 recall).
    'cv_wgt':     dict(features=FEATS_DYN14, inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3, cv_thresh=True, cv_weight=True),
    # cv_loo + per-motor ensemble like 'best' (compare against the robust champion).
    'cv_loo_ens': dict(features=FEATS_DYN14, inject=INJ_V2, peak=PEAK_FULL, model='histgb',
                       postproc=PP_CHAMP3, cv_thresh=True,
                       n_seeds={1: 1, 2: 7, 3: 1, 4: 7, 5: 7, 6: 1}),
}
...
def main():
    ...
    for motor_id in range(1, 7):
        ...
        def make_aug(raw, seed_offset):
            np.random.seed(args.seed * 1000 + motor_id + seed_offset)
            s = synthesize_for_motor(raw, motor_id, n_sequences=INJ[motor_id],
                                     peak_low=peak_low, peak_high=peak_high)
            return pd.concat([raw, s], ignore_index=True) if not s.empty else raw

        def fit_one(raw, params, seed_offset):
            aug = make_aug(raw, seed_offset)
            Xa, ya = aug[features], aug[label_col]
            swa = np.ones(len(ya)); swa[ya == 1] = 2.0
            m = build_model(kind, params)
            try:
                m.fit(Xa, ya, sample_weight=swa)
            except (TypeError, ValueError):
                m.fit(Xa, ya)
            return m

        aug0 = make_aug(raw_train, 0)
        ...
        # Step 1: grid search on seed-0 split to pick model PARAMS (threshold re-tuned later).
        ...
        cv_thresh = cfg.get('cv_thresh', False)
        if cv_thresh and len(failure_seqs) >= 2:
            # ── FOCUS threshold-cv: leave-one-fault-out threshold selection ──
            # Each fold holds out ONE real fault sequence + a round-robin share of the
            # normal sequences, then trains on the rest. We choose the threshold/closing
            # by the STANDARD CV-score: compute F1 in EACH fold separately, average those
            # per-fold F1 values, and take the argmax. (Pooling the OOF probabilities and
            # thresholding the concatenation is degenerate here — the temp<->fault
            # inversion makes one fold's probs badly separated, so the pooled F1-max
            # collapses to "predict everything". Averaging per-fold F1 instead recovers a
            # moderate operating point: the inverted fold is low everywhere, the good fold
            # peaks at a sensible threshold.) The final model trains on ALL sequences (CV
            # frees us to use every real fault seq instead of holding one out for val).
            seq_arr = motor_df['test_condition'].to_numpy()
            n_fail  = len(failure_seqs)
            rng     = np.random.RandomState(args.seed * 1000 + motor_id)
            norm_sh = list(normal_seqs); rng.shuffle(norm_sh)
            norm_chunks = [norm_sh[i::n_fail] for i in range(n_fail)]
            fold_probs  = []   # list of (y_fold, p_fold, weight) with both classes present
            fold_models = []   # the per-fold models (for cv_deploy='folds')
            cv_weight   = cfg.get('cv_weight', False)
            for i, f in enumerate(failure_seqs):
                val_cv = [f] + norm_chunks[i]
                raw_cv = motor_df[~motor_df['test_condition'].isin(val_cv)]
                m_cv   = fit_one(raw_cv, best_params, 0)
                fold_models.append(m_cv)
                vmask  = np.isin(seq_arr, val_cv)
                yf = motor_df.loc[vmask, label_col].to_numpy()
                if len(np.unique(yf)) > 1:
                    pf = m_cv.predict_proba(motor_df.loc[vmask, features])[:, 1]
                    # cv_weight: weight a fold's F1 by its fault-row count, so motors whose
                    # fault mass is concentrated in ONE sequence (M2/M4) let that sequence
                    # drive the threshold instead of being diluted by many fault-free folds.
                    w = float((yf == 1).sum()) if cv_weight else 1.0
                    fold_probs.append((yf, pf, w))
            thr, closing, val_f1 = 0.5, True, -1.0
            if fold_probs:
                for t in thresholds:
                    for use_closing in (False, True):
                        f1s, ws = [], []
                        for yf, pf, w in fold_probs:
                            base = (pf >= t).astype(int)
                            pred = (binary_closing(base, structure=np.ones(5)).astype(int)
                                    if use_closing else base)
                            f1s.append(f1_score(yf, pred, pos_label=1, zero_division=0))
                            ws.append(w)
                        s = float(np.average(f1s, weights=ws))
                        if s > val_f1:
                            val_f1, thr, closing = s, t, use_closing
            # cv_deploy='folds' -> deploy the cross-fold ensemble itself (each model has
            # seen all-but-one fault seq), so the test-time probability scale matches the
            # per-fold probs the threshold was tuned on. 'all' -> retrain on every seq
            # (sharper probs -> threshold transfers poorly; see cv_loo regression).
            if cfg.get('cv_deploy', 'all') == 'folds':
                ens = fold_models
            else:
                full_raw = motor_df
                ens = [fit_one(full_raw, best_params, 0)]
                for k in range(1, n_seeds):
                    ens.append(fit_one(full_raw, best_params, k * 100000))
        else:
            # ── Original single-split path (unchanged) ──
            # Step 2: build the ensemble (K injection seeds, best params reused).
            ens = [model0]
            for k in range(1, n_seeds):
                ens.append(fit_one(raw_train, best_params, k * 100000))
